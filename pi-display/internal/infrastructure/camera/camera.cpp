...
namespace infrastructure {
    void Camera::createCamera(const CameraConfig &config) {
        openCamera();
    ...
    void Camera::configureViewFinder(const CameraConfig &config) {
    ...
    void Camera::setupBuffers(const int &camera_buffer_count) {
    ...
    void Camera::Start() {
        ...
        setControls();
        // set done callback
        _camera->requestCompleted.connect(this, &Camera::requestComplete);
        // queue requests
    ...
    void Camera::requestComplete(Request *request) {
        ...
        auto out_ptr = std::shared_ptr<void>(out_buffer, [this, self, out_buffer](void *p) {
            this->queueRequest(out_buffer);
        });
        _manager->PostMessage(DisplayMessageType::CAMERA_IMAGE, std::move(out_ptr));
    }

    void Camera::queueRequest(CameraBuffer *buffer) {
    ...
    void Camera::Stop() {
        ...
        if (_camera) {
            _camera->requestCompleted.disconnect(this, &Camera::requestComplete);
        }
